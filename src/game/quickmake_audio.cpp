...
void stopMIDIInstrument (midi_channel *channel, unsigned char midiNote) {
    for (int i = 0; i < channel->playingSounds.numValues; ++i) {
        synth_sound_NEW *currentSound = &channel->playingSounds.values[i];
        if (currentSound->id == midiNote) {
            stopSynthSound(currentSound);
        }
    }
}

void playMIDIInstrument (midi_channel *channel, unsigned char midiNote, unsigned char velocity) {
    if (velocity == 0){
        stopMIDIInstrument(channel, midiNote);
    }
    else {
        assert(channel->playingSounds.numValues < NUM_PITCHES);

        // TODO(ebuchholz): check for existing sound with same note
        synth_sound_NEW *sound = 0;
        for (int i = 0; i < channel->playingSounds.numValues; ++i) {
            synth_sound_NEW *playingSound = &channel->playingSounds.values[i];
            if (playingSound->id == midiNote) {
                sound = playingSound;
                break;
            }
        }

        float velocityNormalized = (float)velocity / 127.0f;

        if (sound) {
            restartSound(sound, channel->instrument, midiNoteFrequencies[midiNote], velocityNormalized);
        }
        else {
            synth_sound_NEW newSound = {};
            newSound.id = midiNote;
            startSound(&newSound, channel->instrument, midiNoteFrequencies[midiNote], velocityNormalized);
            listPush(&channel->playingSounds, newSound);
        }
    }
}
...
void instrumentBendPitch (midi_channel *channel, unsigned int amount) {
    for (int i = 0; i < channel->playingSounds.numValues; ++i) {
        synth_sound_NEW *playingSound = &channel->playingSounds.values[i];
        double newHz = 440.0 * 
                      pow(2.0, ((double)playingSound->id - 69.0)/12.0 + ((double)amount - 8192.0)/(4096.0 * 12.0));
        playingSound->pitch = (float)newHz;
    }
}
    ...
    for (int i = 0; i < sampleCount; ++i) {
                    ...
                    if (eventCode < MIDI_EVENT_CODE_SYSTEM_EXCLUSIVE) {
                        ...
                        switch (upperEventCode) {
                            case MIDI_EVENT_CODE_NOTE_OFF_EVENT: {
                                ...
                                stopMIDIInstrument(midiChannel, data0);
                                ++trackPlayback->cursor;
                            } break;
                            case MIDI_EVENT_CODE_NOTE_ON_EVENT: {
                                ...
                                midi_channel *midiChannel = &bgmState->channels[channel];
                                if (channel == 9) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_SNARE_DRUM;
                                }
                                playMIDIInstrument(midiChannel, data0, data1);
                            } break;
                            ...
                            case MIDI_EVENT_CODE_PROGRAM_CHANGE: {
                                ...
                                if (data0 < 24) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_PIANO;
                                }
                                // guitar/bass
                                else if (data0 < 40) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_GUITAR;
                                }
                                // violin/ensemble
                                else if (data0 < 56) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_VIOLIN;
                                }
                                // brass
                                else if (data0 < 64) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_TRUMPET;
                                }
                                // wind
                                else if (data0 < 80) {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_FLUTE;
                                }
                                // drums
                                else {
                                    midiChannel->instrument = SYNTH_INSTRUMENT_TYPE_PIANO;
        ...
        if (allTracksComplete) {
            ...
            for (int channelIndex = 0; channelIndex < NUM_MIDI_CHANNELS; ++channelIndex) {
                ...
                for (int soundIndex = 0; soundIndex < channel->playingSounds.numValues; ++soundIndex) {
                    synth_sound_NEW *sound = &channel->playingSounds.values[soundIndex];
        ...
        for (int channelIndex = 0; channelIndex < NUM_MIDI_CHANNELS; ++channelIndex) {
            midi_channel *channel = &bgmState->channels[channelIndex];
            for (int soundIndex = channel->playingSounds.numValues - 1; soundIndex >= 0; --soundIndex) {
                synth_sound_NEW *sound = &channel->playingSounds.values[soundIndex];

                float soundValue = updateSynthSound(&gameSounds->synthManager.instrumentOutputNodes[sound->instrument], sound, (float)dt);
                soundValue *= sound->velocity;
                // TODO(ebuchholz): mix things nicely
                sampleValue += soundValue * 0.1f;

                if (sound->ended) {
                    listSplice(&channel->playingSounds, soundIndex);
                }
            }
        }
...
void initGameSounds (game_sounds *gameSounds, memory_arena *memory) {
    for (int i = 0; i < NUM_MIDI_CHANNELS; ++i) {
        midi_channel *channel = &gameSounds->bgmState.channels[i];
        channel->playingSounds = synth_sound_NEWListInit(memory, NUM_PITCHES);
    }
}
