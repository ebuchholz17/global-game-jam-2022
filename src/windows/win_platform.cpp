...
int WINAPI WinMain (HINSTANCE instance, HINSTANCE prevInstance, LPSTR commandLine, int showCode) {
    ...
    if (RegisterClassA(&windowClass)) {
        ...
        if (window) {
            ...
            LARGE_INTEGER lastUpdateCounter = lastCounter;
            ...
            while (programRunning) {
                ...
                LARGE_INTEGER updateCounter;
                QueryPerformanceCounter(&updateCounter);

                long long updateElapsed = updateCounter.QuadPart - lastUpdateCounter.QuadPart;
                float updateElapsedSeconds = (float)updateElapsed / (float)perfCountFrequency;
                updateAccumulator += updateElapsedSeconds;

                lastUpdateCounter = updateCounter;


                while (updateAccumulator >= DELTA_TIME) {
                    // QQQ need separate call to draw
                    //renderCommands.memory.size = 0;
                    //memset(renderCommands.memory.base, 0, renderCommands.memory.capacity);
                    //memset(gameMemory.tempMemory, 0, gameMemory.tempMemoryCapacity);

                    updateGame(&input, &gameMemory, &renderCommands, &options, &platformTriggers);
                    updateAccumulator -= DELTA_TIME;
                    resetInput(&input);
                }

                drawGame(&gameMemory, &renderCommands, &options);
